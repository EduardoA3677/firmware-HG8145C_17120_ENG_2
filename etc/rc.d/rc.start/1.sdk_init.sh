...
# 需要知道芯片类型，提前加载hw_module_common.ko
...
if [ $tempChipType -eq 5115 ]; then
    ...
    if [ -e /var/SD5116L.txt ]; then
    	echo "insmod hi_l3.ko napt_ext_ipv4_mode=2 napt_ext_ipv6_mode=2"
    	insmod hi_l3.ko napt_ext_ipv4_mode=2 napt_ext_ipv6_mode=2
    else
    	echo "insmod hi_l3.ko napt_ext_ipv4_mode=3 napt_ext_ipv6_mode=3"
    	insmod hi_l3.ko napt_ext_ipv4_mode=3 napt_ext_ipv6_mode=3
    fi
fi    

if [ ${var_xpon_mode} == "2" ] || [ ${var_xpon_mode} == "6" ] || [ ${var_xpon_mode} == "7" ] || [ ${var_xpon_mode} == "4" ] || [ ! -e /var/SD5116L.txt ] ;  then 
	insmod hi_oam.ko
fi
...
if [ $qoe_enble = 1 ];then
	if [ -e /mnt/jffs2/jscmcc.qosmonloader.cpk ]; then
		...
		#mv /mnt/jffs2/vixtel/xrobot /mnt/jffs2/vixtel/monitor.wan.bin
...
[ -f /lib/modules/wap/hw_ptp_wifi.ko ] && insmod /lib/modules/wap/hw_ptp_wifi.ko
[ -f /lib/modules/wap/hw_ptp_mobile.ko ] && insmod /lib/modules/wap/hw_ptp_mobile.ko
[ -f /lib/modules/wap/hw_ptp_sim.ko ] && insmod /lib/modules/wap/hw_ptp_sim.ko
...
then    
    ...
    [ -f /lib/modules/wap/capture.ko ] && insmod /lib/modules/wap/capture.ko
fi
...
[ -f /lib/modules/wap/rawip_adpt.ko ] && insmod /lib/modules/wap/rawip_adpt.ko

insmod /lib/modules/wap/video_diag_adpt.ko
insmod /lib/modules/wap/btv_adpt.ko

echo "Loading BBSP l2tp modules: "
[ -f /lib/modules/linux/kernel/net/l2tp/l2tp_core.ko ] && insmod /lib/modules/linux/kernel/net/l2tp/l2tp_core.ko
[ -f /lib/modules/linux/kernel/net/l2tp/l2tp_ppp.ko ] && insmod /lib/modules/linux/kernel/net/l2tp/l2tp_ppp.ko
...
then    
    ...
    echo -n "Start VOICE ..."
        [ -f /bin/voice_h248 ] && $task_bind voice_h248 &
	[ -f /bin/voice_sip ] && $task_bind voice_sip &
	[ -f /bin/voice_mgcp ] && $task_bind voice_mgcp &
	[ -f /bin/voice_h248sip ] && $task_bind voice_h248sip &
fi

if [ $en = 1 ];then
#    echo -n "Start cwmp..."
    $task_bind cwmp& break;
fi
...
while true; do sleep 40 ; $task_bind mu& break; done &

# 延后启动的进程
if [ -f /bin/apm ] ; then
	while true; do sleep 30; $task_bind apm& break; done &
fi
...
if [ -c "/dev/ubi0_14" ]; then
	...
	while true; do sleep 55; osgi_proxy& break; done &
    ...
    mkdir -p /mnt/jffs2/app;chown :cplugin /mnt/jffs2/app;chmod 750 /mnt/jffs2/app;

	chown -R osgi:osgi /mnt/jffs2/app/osgi;
	chown -R osgi:osgi /var/osgi;
	chown -R osgi:osgi /var/felix-temp;
	chown -R osgi:osgi /tmp; 
	chmod 777 /tmp;
	
	cp -rf /etc/wap/osgi_bundlelist.info /var/osgi/bundlelist.info;
	chown osgi:osgi /var/osgi/bundlelist.info;
...
chown :cplugin /mnt/jffs2/app/cplugin;
